      ...
      {sections.map((section, index) => (
        <section className="story-page" id={section.id} key={section.id}>
          ...
          <div className="story-copy">
            ...
            {section.comparison && (
              <div className="mindset-table" role="table" aria-label={inChinese ? "对比表" : "Comparison table"}>
                <div className="mindset-head" role="row">
                  <strong role="columnheader">{section.comparison.leftTitle}</strong>
                  <strong role="columnheader">{section.comparison.rightTitle}</strong>
                </div>
                {section.comparison.rows.map(([left, right]) => (
                  <div className="mindset-row" role="row" key={left}>
            ...
            {section.choice && (
              <div className="choice-block">
                ...
                <div role="status" aria-live="polite">
                  {answers[section.id] && answers[section.id] !== section.choice.answer && <p className="try-again">{inChinese ? "想得不错。再看看这一章，然后试一次。" : "Good thinking. Look back at the chapter and try once more."}</p>}
                  {answers[section.id] === section.choice.answer && <p className="right-answer">{inChinese ? "答对了，推理得很好！" : "That's it. Nice reasoning!"}</p>}
                </div>
              </div>
            )}
          </div>
